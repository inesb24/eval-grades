...
grades_pivot <-
  ...
  summarise(average_course = mean(grade)) |>
  pivot_wider(names_from = "course", values_from = "average_course")
grades_pivot |>
...
```{r}
grades_pivot |>
  ggplot(aes(x = `Potion Brewing and Herbology`, y = `Swordsmanship and Martial Arts`, colour = `Swordsmanship and Martial Arts`)) + geom_point(alpha = 0.5) +
...
necro_correlation <-
  grades_pivot |>
  group_by(group) |>
  summarise(corr = cor(`Necromancy and Spirit Summoning`,`Illusion and Enchantment`))
```
...
necro_correlation |>
  filter(corr>0) |>
  ggplot(grades_pivot, aes(y = `Necromancy and Spirit Summoning`, x = `Illusion and Enchantment`)) + geom_point()
```
